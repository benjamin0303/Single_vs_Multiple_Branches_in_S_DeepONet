# Thermo-mechanical (coupled) - single-branch model evaluated on the multi-branch test split (paired comparison plots)
#
# Code release for:
...
for idx , fraction_train in enumerate([ 0.8 ]):
        ...
        while(1):
                ...
                model_2br_2 = np.load("model_2br_2.npz")
                test_case = model_2br_2['e']
                print("test_case.shape = ", test_case.shape)
                ...
                losshistory, train_state = model.train(iterations=0, batch_size=batch_size, model_restore_path="./Models/model_1br_2-310000.ckpt")
                ...
                print()
                np.savez_compressed('model_1br_infer_2br_'+str(count)+'.npz', a=Org_temp_test, b=Org_stress_test, c=Org_temp_pred, d=Org_stress_pred, e=test_case)
                ...
                print()
                print()
                if count > 0  or org_error_s.mean() < 0.10:
                        break
                
                count = count + 1

        plt.hist( error_s.flatten() , bins=25 )
        plt.savefig('Stress_Err_hist_1br_infer_2br_'+str(count)+'.jpg' , dpi=300)

        plt.hist( error_t.flatten() , bins=25 )
        plt.savefig('Temp_Err_hist_1br_infer_2br_'+str(count)+'.jpg' , dpi=300)
