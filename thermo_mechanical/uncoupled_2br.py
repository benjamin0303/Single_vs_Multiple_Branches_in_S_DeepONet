# Thermo-mechanical (uncoupled multiphysics) - multi-branch S-DeepONet / MIONet
#
# Code release for:
...
data_loc = 'data/thermo_mechanical/uncoupled'
...
disp_filtered = np.load(os.path.join(data_loc, 'disp_N_train.npy'))
...
stress_data = np.load(os.path.join(data_loc, 'stress_time_steps_3000.npy'))
...
for idx , fraction_train in enumerate([ 0.8 ]):
        ...
        while(1):
                ...
                losshistory, train_state = model.train(iterations=110000, batch_size=batch_size, model_save_path="./Models/model_2br_"+str(count)) 
                ...
                print()
                np.savez_compressed('model_2br_'+str(count)+'.npz', a=Org_temp_test, b=Org_stress_test, c=Org_temp_pred, d=Org_stress_pred, e=test_case)
