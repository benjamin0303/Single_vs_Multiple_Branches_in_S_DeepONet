# Thermo-electrical (uncoupled multiphysics) - single-branch S-DeepONet
#
# Code release for:
...
# %%
training_data_T = np.load(
    'data/thermo_electrical/uncoupled/elec[phi(t)]_thermal_t_uncouple_thermal.npz')

training_data_phi = np.load(
    'data/thermo_electrical/uncoupled/elec[phi(t)]_thermal_t_uncouple_elet.npz')

x_grid = training_data_T['x_grid']                     # (255, )
t_grid = training_data_T['t_grid']                     # (101, )
x, t = np.meshgrid(x_grid, t_grid)                     # (101, 255), (101, 255)
grid_input = np.stack([x.ravel(), t.ravel()], axis=1)  # (Nt*Nx,2)
rhoe = training_data_phi['rho_e_all']                  # (5000, 101)
Qext = training_data_T['Q_ext_all'][:, :101]           # (5000, 101), originally (5000, 255) but we only use the first 101 time steps
T_solutions = training_data_T['T_solutions']           # (5000, 101, 255)
T_shift, T_scale = T_solutions[:, -1, :].mean(), T_solutions[:, -1, :].std()
T_sol = (T_solutions - T_shift) / T_scale

phi_solutions = training_data_phi['phi_solutions']     # (5000, 101, 255)
phi_shift, phi_scale = phi_solutions[:, -1, :].mean(), phi_solutions[:, -1, :].std()
phi_sol = (phi_solutions - phi_shift) / phi_scale

num_samples = T_solutions.shape[0]
target_T_phi = np.stack([T_sol.reshape(num_samples, -1), phi_sol.reshape(num_samples, -1)], axis=-1) # (5000, 101*255, 2)

rhoe_shift, rhoe_scale = rhoe.mean(), rhoe.std()
rhoes = (rhoe - rhoe_shift) / rhoe_scale
Qext_shift, Qext_scale = Qext.mean(), Qext.std()
Qexts = (Qext - Qext_shift) / Qext_scale

inputs = np.stack([Qexts, rhoe], axis=-1)            # (5000, 101, 2)

n_step         = inputs.shape[1]                     # 101
n_cases        = inputs.shape[0]                     # 5000
n_nodes        = target_T_phi.shape[1]//n_step       # 255
N_input_fn     = 2
HIDDEN         = 100
m              = n_step                              # 101
...
xy_train_testing = grid_input.reshape(n_step, n_nodes, 2)[-1, :, :1]       # (225, 1)
target_T_phi = target_T_phi.reshape(n_cases, n_step, n_nodes, N_component) # (5000, 101, 255, 2)


def T_inv(T):
    return T * T_scale + T_shift


def phi_inv(phi):
    return phi * phi_scale + phi_shift
...
# %%
fraction_train        = 0.8
N_valid_case          = len(inputs)                                                # 5000
N_train               = int(N_valid_case * fraction_train)                         # 4000
train_case            = np.random.choice(N_valid_case, N_train, replace=False)     # 4000
test_case             = np.setdiff1d(np.arange(N_valid_case), train_case)          # 1000

u0_train              = inputs[train_case]                                         # (4000, 101, 2)
u0_testing            = inputs[test_case]                                          # (1000, 101, 2)
s_train               = target_T_phi[train_case, -1:, :, :]                        # (4000, 1, 255, 2)
s_testing             = target_T_phi[test_case, -1:, :, :]                         # (1000, 1, 255, 2)

x_train               = (u0_train, xy_train_testing)                               # (4000, 101, 2), (255, 1)
y_train               = s_train                                                    # (4000, 1, 255, 2)
x_test                = (u0_testing, xy_train_testing)                             # (1000, 101, 2), (255, 1)
y_test                = s_testing                                                  # (1000, 1, 255, 2)

data = TripleCartesianProd(x_train, y_train, x_test, y_test)
my_act1 = "tanh"
branch = tf.keras.models.Sequential([
    tf.keras.layers.GRU(units=256, batch_input_shape=(batch_size, m, N_input_fn), # (B, 101, 2)
...
losshistory, train_state = model.train(
    iterations=310000, batch_size=batch_size, model_save_path="./Models/model_1br_uncouple/model")
np.save('./Models/model_1br_uncouple/losshistory.npy', losshistory)
...
y_pred_T = T_inv(y_pred[:, 0, :, 0])
y_pred_S = phi_inv(y_pred[:, 0, :, 1])
y_true_T = T_inv(data.test_y[:, 0, :, 0])
y_true_S = phi_inv(data.test_y[:, 0, :, 1])
np.savez('./Models/model_1br_uncouple/predictions.npz', x_grid=data.test_x[1][:, 0],
