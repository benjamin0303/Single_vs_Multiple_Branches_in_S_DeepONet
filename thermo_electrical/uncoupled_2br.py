# Thermo-electrical (uncoupled multiphysics) - multi-branch S-DeepONet / MIONet
#
# Code release for:
...
x_grid = training_data_T['x_grid']
t_grid = training_data_T['t_grid']
x, t = np.meshgrid(x_grid, t_grid)
grid_input = np.stack([x.ravel(), t.ravel()], axis=1)  # (Nt*Nx,2)
rhoe = training_data_phi['rho_e_all']
Qext = training_data_T['Q_ext_all'][:, :101]
T_solutions = training_data_T['T_solutions']
T_shift, T_scale = T_solutions[:, -1, :].mean(), T_solutions[:, -1, :].std()
T_sol = (T_solutions - T_shift) / T_scale

phi_solutions = training_data_phi['phi_solutions']
phi_shift, phi_scale = phi_solutions[:, -1,
                                     :].mean(), phi_solutions[:, -1, :].std()
phi_sol = (phi_solutions - phi_shift) / phi_scale

num_samples = T_solutions.shape[0]
target_T_phi = np.stack([T_sol.reshape(
    num_samples, -1), phi_sol.reshape(num_samples, -1)], axis=-1)
...
inputs = np.stack([Qexts, rhoe], axis=-1)
...
# %%
net = DeepONetCartesianProd([m, branch, branch2], [
                            trunk], my_act2, "Glorot normal")
...
losshistory, train_state = model.train(
    iterations=310000, batch_size=batch_size, model_save_path="./Models/model_2br_uncouple/model")
np.save('./Models/model_2br_uncouple/losshistory.npy', losshistory)
...
np.savez('./Models/model_2br_uncouple/predictions.npz', x_grid=data.test_x[1][:, 0],
