# Reaction-Diffusion (single physics) - multi-branch DeepONet / MIONet (FNN branches)
#
# Code release for:
...
N_component = 1 # of predicted solution fileds, one only solution 
...
class DeepONetCartesianProd(dde.maps.NN):
        ...
        def __init__(
                ...
                self.branch2 = layer_sizes_branch[2]
        ...
        def call(self, inputs, training=False):
                ...
                x_func1 = self.branch(x_func[:,:,0]) # [ bs , HD , N_TS ]
                x_func2 = self.branch2(x_func[:,:,1]) # [ bs , HD , N_TS ]
                ##
                x_func = tf.multiply( x_func1 , x_func2 )
...
hf=h5py.File('data/reaction_diffusion/RD_gauss_cov40k.h5', 'r')
x_grid=hf['x_grid'][:].squeeze().astype(np.float64)# shape (Nx,)
t_grid = hf["t_grid"][:].squeeze().astype(np.float64)
...
ICs_raw = hf["u0s"][:].astype(np.float64)
Ks_raw = hf["Ks"][:].astype(np.float64)
solutions_raw = hf["solutions"][:].astype(np.float64)  # shape (N, Nt, Nx)
print("solutions_raw.shape = ", solutions_raw.shape)

solutions_raw=solutions_raw.reshape(solutions_raw.shape[0], 1, nodes)
print("new solutions_raw.shape = ", solutions_raw.shape)
...
# Scale
scalerICs = MinMaxScaler()
scalerICs.fit(ICs_raw)
ICs = scalerICs.transform(ICs_raw)
print("ICs.shape = ", ICs.shape) 
print('----------')


# Scale
scalerKs = MinMaxScaler()
scalerKs.fit(Ks_raw)
Ks = scalerKs.transform(Ks_raw)
...
Heat_Amp = np.stack( [ICs[:n_cases,:],Ks[:n_cases,:]] , axis=-1 )

# s
data_s = solutions_raw[:n_cases, -1, :]
...
print("scaled_stress.shape = ", scaled_stress.shape)
print('----------')
##exit()


Temp = np.zeros((n_cases , N_output_frame , nodes , N_component) )
Temp[:, -1, :, 0] = scaled_stress
...
for idx , fraction_train in enumerate([ 0.8 ]):
        ...
        while(1):
                ...
                branch = tf.keras.models.Sequential([
                        # Flatten input to (batch_size, Nx * 1) for dense layers
                            tf.keras.layers.Flatten(input_shape=(Nx, 1)),
    
                        # Fully connected layers (customize number of units as needed)
                        ...
                        # Output layer with (batch_size, hd * 1) shape
                            tf.keras.layers.Dense(HIDDEN, activation='linear'),  # Ensures output has (batch_size, hd) shape

                        # Reshape to (batch_size, hd, 1) for the final output
                            tf.keras.layers.Reshape([HIDDEN, N_output_frame]),
                                                                ])
                branch.summary()

                branch2 = tf.keras.models.Sequential([
                        # Flatten input to (batch_size, Nx * 1) for dense layers
                            tf.keras.layers.Flatten(input_shape=(Nx, 1)),
                ...
                net = DeepONetCartesianProd(
                                [m, branch, branch2], [trunk], my_act2, "Glorot normal")
                ...
                def metric1( y_train , y_pred ):
                        ...
                        y_train_original = scalerS.inverse_transform(y_train[:,0,:,0])
                        ##y_pred_original = scalerS.inverse_transform(y_pred[:,0,:,1])
                        y_pred_original = scalerS.inverse_transform(y_pred[:,0,:,0])
                ...
                losshistory, train_state = model.train(iterations=100000, batch_size=batch_size, model_save_path="TrainFrac_"+str(idx) )
                ...
                Org_stress_test = scalerS.inverse_transform(y_test[:,0,:,0])
                ##Org_temp_pred = scalerT.inverse_transform(y_pred[:,0,:,0])
                Org_stress_pred = scalerS.inverse_transform(y_pred[:,0,:,0])
                print('Successful to convert to original forms')
                print('xy_train_testing.shape = ', xy_train_testing.shape)
                print('u0_testing.shape = ', u0_testing.shape)
                print('Org_stress_pred.shape = ', Org_stress_pred.shape)
                print('Org_stress_test.shape = ', Org_stress_test.shape)
                np.savez_compressed('rd_fnn_test_results_2br_mionet.npz', grid=xy_train_testing,inputs_2br=u0_testing, pred_2br=Org_stress_pred, target_2br=Org_stress_test)
                print('Saving all successful')
                print()
                ##np.savez_compressed('Ver_4_Org_y_try_'+str(count)+'.npz', a=Org_temp_test, b=Org_stress_test, c=Org_temp_pred, d=Org_stress_pred, e=test_case)
                print('Saving all successful')
                ...
                error_s = err(y_test[:, 0, :, 0], y_pred[:, 0, :, 0])
                ...
                print()
                print()
                if count > 3  or org_error_s.mean() < 0.10:
                        break
